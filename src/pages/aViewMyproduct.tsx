...
const UserView = () => {
  ...
  const isMyProductsPage = !creatorId; // Quando não há creatorId, é a página de produtos comprados
  const {
    messages,
    sendMessage
  ...
  useEffect(() => {
    const loadCreatorData = async () => {
      // Se for a página de produtos comprados (/myproducts), não precisa carregar dados do criador
      if (isMyProductsPage) {
    ...
    loadCreatorData();
  }, [creatorId, isViewingCreatorPage, isCreator, isMyProductsPage]);
  ...
  // Se for a página de produtos comprados (/myproducts), mostrar apenas os produtos comprados
  if (isMyProductsPage) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-secondary to-background p-4">
        {/* Header simplificado para página de produtos */}
        <div className="text-center mb-8">
          <div className="flex flex-col items-center gap-1">
            <div className="text-center">
              <h1 className="text-xl sm:text-2xl font-bold font-sans tracking-wide">
                <span className="bg-gradient-to-r from-orange-400 via-pink-500 to-purple-600 bg-clip-text text-transparent">Meus</span>
                <span className="bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent ml-2">Produtos</span>
              </h1>
              <p className="text-sm text-muted-foreground">Conteúdo que você comprou</p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto space-y-4">
          ...
          {/* Header com créditos do usuário */}
          ...
          {isLoggedIn ? (
            ...
            <Card className="p-8 text-center">
              <div className="space-y-4">
                ...
                <h3 className="text-lg font-medium">Faça login para ver seus produtos</h3>
                <p className="text-muted-foreground">
                  Você precisa estar logado para visualizar o conteúdo que comprou.
                </p>
                <GoogleAuthButton onLoginSuccess={() => setShowUserDialog(true)} />
              </div>
            </Card>
